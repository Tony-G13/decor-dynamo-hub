    ...
    <section className="py-20">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 relative">
          {/* Connection Lines */}
          <div className="hidden md:block absolute top-24 left-0 right-0 h-0.5 bg-gradient-to-r from-primary via-secondary to-tertiary opacity-30"></div>
          
          {steps.map((step, index) => (
            <div key={step.number} className="relative">
              <Card 
                className="text-center group hover:shadow-xl transition-all duration-300 hover:-translate-y-4 border-0 animate-scale-in"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <CardContent className="p-6">
                  {/* Step Number */}
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 w-8 h-8 bg-primary text-primary-foreground rounded-full flex items-center justify-center text-sm font-bold group-hover:scale-110 transition-transform">
                    {step.number}
                  </div>
                  
                  <div className="mt-4 mb-6">
                    <step.icon className="h-12 w-12 mx-auto text-primary group-hover:scale-110 transition-transform duration-300" />
                  </div>
